...
export class RoomsTableComponent implements OnInit {
  ...
  loadData() {
    zip([
      ...
      this.hotelsSvc.list(),
      this.typesSvc.list(),
      this.reservationSvc.getToday()
    ]).subscribe(([rooms, hotels, types, reservations]) => {
      this.hotels = hotels;
      this.types = types;
  ...
  // Helper method to check if room has confirmed reservation today
  hasConfirmedReservationToday(roomId: number): boolean {
    return this.todayReservations.some(reservation =>
      reservation.room?.room_id === roomId && reservation.status === 'CONFIRMED'
    );
  }
    ...
    columnDefs: [
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          ...
          const hasConfirmedReservation = this.hasConfirmedReservationToday(room.room_id!);

          if (room.res_status === 'MAINTENANCE') {
  ...
  save() {
    ...
    this.roomsSvc.create(payload).subscribe({
      next: (created) => {
        ...
        // Refresh today's reservations to update room status
        this.reservationSvc.getToday().subscribe(reservations => {
          this.todayReservations = reservations;
